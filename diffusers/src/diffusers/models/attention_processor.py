...
class JointAttnProcessor2_0:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if hasattr(self, "feat_cache") and self.feat_cache is not None:
            self.img_feat_cache = hidden_states
            self.txt_feat_cache = encoder_hidden_states 
...
class AuraFlowAttnProcessor2_0:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.attn_cache is not None:
            attn_scores = (query @ key.transpose(-1, -2) / math.sqrt(head_dim))
            if self.softmax:
                attn_scores = attn_scores.softmax(dim=-1)
                self.attn_cache = attn_scores.mean(dim=1)
            else:
                self.attn_cache = attn_scores.clone()
                attn_scores = attn_scores.softmax(dim=-1)#.half()
            
        # Attention.
        # hidden_states = F.scaled_dot_product_attention(
        #     query, key, value, dropout_p=0.0, scale=attn.scale, is_causal=False
        # )
        hidden_states = attn_scores @ value
        hidden_states = hidden_states.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)
        hidden_states = hidden_states.to(query.dtype)
        ...
        if self.attn_cache is not None:
            ...
            if self.softmax:
                ...
                if self.head == None:
                    ...
                    self.attn_cache = attn_scores[:, self.head, :, -4096:]
            else:
                if self.keep_head:
                    self.attn_cache = attn_scores[:, :, -4096:, :-4096]
                else:
                    self.attn_cache = attn_scores.mean(dim=1)[:, -4096:, :-4096]
                # self.attn_cache = attn_scores.clone()
                # attn_scores = attn_scores.softmax(dim=-1)#.half()
        ...
        if self.attn_cache is not None:
            ...
            if self.softmax:
                ...
                if self.head == None:
                    ...
                    self.attn_cache = attn_scores[:, self.head]
            else:
                if self.keep_head:
                    self.attn_cache = attn_scores[:, :, -4096:, :-4096]
                else:
